// src/components/CustomersCarousel.tsx
...
const CustomersCarousel = () => {
  ...
  // Função para mover o carousel automaticamente
  ...
  useEffect(() => {
    ...
    // Intervalo de 3 segundos para mudança automática
    const intervalId = setInterval(autoSlide, 3000);

    // Pausar autoplay quando o usuário interage com o carousel
    const handlePointerDown = () => {
      clearInterval(intervalId);
    };

    const handlePointerUp = () => {
      // Reiniciar autoplay após 5 segundos de inatividade
      setTimeout(() => {
        ...
        <CarouselContent className="-ml-2 md:-ml-4">
          {clients.map((client, index) => (
            <CarouselItem key={index} className="pl-2 md:pl-4 basis-1/2 md:basis-1/3 lg:basis-1/4 xl:basis-1/5">
              <div className="p-1">
                <Card className="border-gray-200 hover:border-gray-300 transition-colors duration-200 bg-white">
                  <CardContent className="flex aspect-[8/3] items-center justify-center p-4">
                    {client.url !== "#" ? (
                      <a
                        href={client.url}
                        target="_blank"
